...
- [ ] Correr `doctor-consultation` reescrito (2026-07-28, verificación dura post-Finalizar) contra dev y confirmar que pasa limpio — ver sección "🔧 Endurecimiento de `consultation.full-flow.spec.js`".
- [ ] Re-confirmar con una 2ª corrida el bug del botón "Consulta" (Tipo/Hospital vacíos, ver sección homónima) antes de reportarlo formalmente a devs — solo 1 corrida hasta ahora.
- [ ] Probar campos obligatorios y selects dependientes en el wizard de **alta de paciente nuevo** (`/Pacientes` → "Agregar paciente" → "Crear perfil de paciente nuevo") — solo se mapeó el flujo (`_explorar_alta_paciente_nuevo*.js`), sin probar aún si se puede avanzar/guardar con campos obligatorios vacíos.
