...
>
> **Última actualización:** 2026-08-17 (**🔍 Corrida completa de la suite oficial de dev (12 specs con asserts duros) con observación metodológica paso a paso, a pedido de Pedro — 4 hallazgos nuevos confirmados (2 de severidad alta), 1 hallazgo abierto sin confirmar, 6 selectores desactualizados corregidos en 2 specs, 1 gap de cobertura de test documentado** — ver sección "🔍 Corrida completa de la suite dev + observación metodológica — 2026-08-17" más abajo para el detalle completo. Resumen: Pedro pidió correr pruebas en dev buscando cambios/errores y que se observara cada paso de las automatizaciones (no solo el resultado final), aplicando la metodología de `CLAUDE.md` §0. Se corrieron, uno por uno con revisión completa del log de cada corrida, los 12 specs de `tests/` con asserts duros (se excluyeron a propósito los `*.explorar.*`, sin asserts por diseño, y los `stress-*`, mucho más largos — no corridos esta vez). **Hallazgos nuevos confirmados:** (1) 🐛 la CSP de dev bloquea el propio script/píxel de Microsoft Clarity (`scripts.clarity.ms`/`c.clarity.ms`, la política solo permite `www.clarity.ms`) — reproducido idéntico 3/3 veces, no afecta funcionalidad clínica pero de paso reveló que `tests/system-health.spec.ts` nunca se había corrido contra dev (sin tocar desde el commit inicial del repo); (2) 🐛 **el bug de Servicios ya confirmado en staging el 2026-08-03 (`saveService` responde 200 con `id_servicio` nuevo pero nunca aparece en `getServices`) también reproduce en dev** (`id_servicio=12`) — sube de "posible bug de staging" a "bug real de backend/BD, no de un entorno específico"; (3) 🐛🔴 **severidad alta, confirmado con evidencia dura: los campos de signos vitales (Peso, Talla, Presión Arterial, Frecuencia Cardiaca, Oxigenación, Frecuencia Respiratoria) aceptan y PERSISTEN valores clínicamente imposibles** (probado con FC=999 bpm: el botón "Guardar" se habilitó y `registerVitalSigns` respondió 200 "registrados correctamente", `consulta_id=6`) — a diferencia de los valores negativos (ya confirmados bloqueados el 2026-07-30), los valores positivos extremos no tienen ningún tope, salvo Temperatura que sí parece tener alguno; (4) 🐛 **regresión en Vacunación: borrar todas las dosis de la cartilla y guardar ya NO persiste** — tras clickear todas las "×" y refrescar, las 32 dosis originales seguían ahí sin ningún cambio (el botón "Guardar cambios" no apareció tras borrar, por lo que el borrado nunca llegó a enviarse al backend); en la corrida de referencia del 2026-07-21 este mismo síntoma se había anotado como hallazgo blando sin bloquear (terminaba en 0 igual) — ahora sí bloquea, es un cambio de comportamiento real. El dato del paciente de prueba (Agustin Tapia) no quedó corrupto, solo sin cambios. **Hallazgo abierto, no confirmado:** durante `ingresos.spec.ts`, la pantalla "Detalle de ingreso" se vio completamente en estado *skeleton* (nunca renderizó) tras abrir un ingreso pendiente, coincidiendo con 2 llamadas a `getFiscalData` con `net::ERR_ABORTED` — no se pudo reproducir en el momento (los datos de dev cambiaron entre corridas) ni confirmar si es el mismo patrón de lentitud ya conocido (documentado 2026-07-14) o una reaparición del bug `DetallePagos` ya "resuelto" el 2026-07-21; queda pendiente investigar con un script dedicado tipo `_investigar_getfilledform_blanco_dev.js`. **Reconfirmación (no nuevo):** el 404 de `getFilledForm` post-Finalizar en `doctor-consultation` (Exploración segmentaria/Aparatos y sistemas) reapareció con timing casi idéntico a corridas anteriores (+95s), sigue sin resolverse, 130s de reintentos no alcanzaron esta vez — el resto de la verificación dura (10 campos) pasó limpio. **Mantenimiento de tests aplicado en vivo** (selectores desactualizados, confirmados contra el DOM real antes de corregir): `reportes.spec.ts` (4 fixes: label de KPI "Número de consultas"→"Citas del período", opción de rango "Últimos mes"→"Último mes", faltaba clic en "Buscar" para aplicar el filtro, regex de "Período" y opción de Estatus "Todos los estatus"→"Todos" — ahora pasa 2/2 limpio) y `subir-estudios.spec.ts` (2 fixes: nombre de paciente `a.font-semibold`→`span.font-semibold`, tab "Consultas" `a`→`button` — antes bloqueaban CUALQUIER progreso del test). **Gaps de cobertura documentados (no son bugs de la app):** `appointments.create.spec.ts` (2do test) no tiene ningún assert duro y busca texto "agendada"/"programada" que no existe en la UI real — confirmado en vivo que la cita sí se crea y se muestra bien, el test simplemente no puede detectarlo; `subir-estudios.spec.ts` sigue sin asserts duros tras el fix de selectores (un "passed" no prueba que el flujo de subir estudios funcione); `consultation.user-errors.spec.js` (5/6 tests fallan) confirma lo ya sabido — el helper compartido `iniciarConsultaDeHoy` solo llena 2 de ~7 campos de signos vitales, dejando "Guardar" deshabilitado; sigue pendiente su reescritura para el "Modo Completo" (trabajo ya identificado, no abordado esta sesión). Scripts de investigación nuevos (raíz, no trackeados): `_diagnostico_csp_clarity_dev.js`, `_diagnostico_agenda_toggle_dev.js`, `_investigar_detalle_ingreso_skeleton_dev.js`, `_verificar_signos_vitales_extremos_dev.js`. Anterior: 2026-08-03 (**🔧 Mejora de detección de errores en la suite Appium de `MediplannerAppiumAutomation/` (app pacientes): 2 bugs reales de la suite encontrados y corregidos, 4 tests reforzados con verificación de persistencia real (incluyendo `test_dependientes_desvincular`, reescrito con el flujo real que aportó Pedro), 2 tests redundantes eliminados** — ver sección "🔧 Mejora de detección de errores — Suite Appium — 2026-08-03" más abajo para el detalle completo. Resumen: a pedido de Pedro se analizó qué tests de la suite Appium se solapaban y qué tan bien detectaban errores reales (no solo si pasaban). Se identificaron candidatos de baja confianza (`test_doctor_search`, subsumido por `test_medicos_flujo_completo`; `test_bitacora_smoke_categoria`, subsumido por los 8 tests dedicados de bitácora) y se eliminaron. Al reforzar `test_home_medicamento_no_tomado`/`test_medicamento_registro_de_toma`/`test_dependientes_agregar` con verificación de persistencia real (reabrir la pantalla y confirmar el dato guardado, no solo que el modal cerró), aparecieron **2 bugs reales de la propia suite** (no de la app), confirmados con evidencia dura (log de Appium + capturas cada 1s): (1) `ocultar_keyboard()` ejecuta `adb shell input keyevent 111` (KEYCODE_ESCAPE), que varios modales de la app tratan como "cancelar" — cerraba el modal completo en vez de solo el teclado, descartando el comentario recién tecleado; (2) `test_dependientes.py` no llamaba `.clear()` antes de `.send_keys()`, y como el proceso de la app no se reinicia entre corridas de pytest, texto de una corrida anterior se concatenaba con el de la nueva (`"Pedro"+"Laura"="PedroLaura"`), produciendo datos inválidos que nunca habilitaban "Continuar". Los 3 tests reforzados pasan limpio tras corregir ambos bugs. Un cuarto test, `test_dependientes_desvincular`, seguía fallando por una causa distinta (el flujo asumido estaba mal — tocar un dependiente en el drawer del titular muestra el perfil PROPIO, no el del dependiente); Pedro aportó el flujo real de la app (cambiar de contexto al dependiente → Perfil → Compartir → desvincular al titular desde ahí) y, con eso más 2 fixes de timing (cambio de contexto no instantáneo + propagación del backend), también quedó pasando limpio — ver sección de detalle para el flujo completo. Anterior: 2026-08-03, antes (**🧪 Porteo a STAGING de los specs de dev con confianza Alta/Media (ajustes-servicios, dashboard, recetas) + fixes de selectores desactualizados + 1 bug real nuevo confirmado en `ajustes.servicios.spec.ts`** — ver sección "🧪 Porteo a STAGING — 2026-08-03" más abajo para el detalle completo. Resumen: se analizaron los 19 specs de `tests/` (dev) y se clasificaron por solidez (Alta/Media/Baja); de los de confianza Alta/Media y actualizados (excluyendo los que aún usan el modelo de pestañas de Consulta, obsoleto desde el 23-jul), se corrieron contra staging los que ya existían ahí (`auth.setup`, `consultation.full-flow`, `ingresos`, `system-health`, `vacunacion-ciclo-completo`) y se portaron los 3 que faltaban (`ajustes.servicios.spec.ts`, `dashboard.spec.js`, `recetas.spec.ts`). Primera corrida completa: 5/14 pasan, 9 fallan. Investigando cada fallo con evidencia real (Playwright directo contra staging, DOM real) se encontró que 7 de 9 eran selectores/rutas desactualizadas (no bugs de plataforma) — corregidos y verificados con corridas repetidas: placeholder de búsqueda de pacientes (`"Buscar Usuarios"` → `"Buscar paciente"`), selector de nombre de paciente en vacunación (`a.font-semibold...` → `span.font-semibold...`, la etiqueta real es `<span>` no `<a>`), `networkidle` que nunca se cumple en `system-health`/`subir-estudios` (reemplazado por `load`+espera, patrón ya usado en el resto del repo), y el paciente hardcodeado `"Agustin Tapia"` en `recetas.spec.ts` (no existe en staging — confirmado contra la API real, 18 pacientes, ninguno con ese nombre; reemplazado por `"Percentil Prueba Prueba"`, que además tiene 21 recetas en 3 páginas, mejor candidato que "Pedro Quijada Anaya" para probar paginación real). **2 hallazgos quedaron abiertos, no son errores del test:** (1) 🐛 bug real confirmado en `ajustes.servicios.spec.ts` — `saveService` responde 200 con un `id_servicio` nuevo pero el servicio nunca aparece en `getServices` después (reproducido 2 veces, una vez vía el test automatizado y otra en investigación manual directa contra la API); (2) la cuenta de staging (`dr@rym-solutions.com`) queda atascada en el wizard de onboarding "Configuración de tu cuenta" (Paso 3/6, Consultorios) y la SPA rebota de vuelta a `/wizard/onboarding` unos segundos después de dismissarlo con "Configurar más tarde" — hace que `dashboard.spec.js` sea flaky verificando el contenido real de las tarjetas KPI; requiere que Pedro complete el wizard manualmente en staging (no se prueba el wizard en sí, por instrucción existente). Estado final tras los fixes: **12/14 specs pasan** (excluyendo `doctor-consultation`, dejado fuera del alcance de esta sesión por pedido explícito de Pedro — el hallazgo de `getFilledForm` en blanco tras 130s ya documentado el 2026-07-31 no se re-investigó). Anterior: 2026-07-31 (**🧪 Porteo y primera corrida en STAGING del `consultation.full-flow.spec.js` endurecido (Modo Completo)** — ver sección "🧪 Porteo a STAGING — 2026-07-31" más abajo para el detalle completo. Resumen: el spec de staging estaba muy desactualizado (versión de pestañas, previa al rediseño "Modo Completo" del 2026-07-23 en dev) — se porteó la versión endurecida completa (checklists Normal/Anormal + verificación dura post-Finalizar) y se agregó `auditarPantalla()` a `e2e/utils.js` de staging (faltaba, solo estaba en dev). La corrida **FALLÓ** con evidencia real de esta corrida (no descartada): (1) **posible bug nuevo en staging** — el diagnóstico CIE-10 y la Impresión diagnóstica quedaron **vacíos tras Finalizar** (`getConsultation`: `diagnosticos: 0 elementos`, `impresion_diagnostico: ""`), pese a haberse llenado y a que el log muestra que sí se seleccionaron; se observó que seleccionar el CIE-10 dispara un `editConsultation` automático INMEDIATO (antes de terminar de llenar el resto de Diagnóstico) y que el `addDiagnosis` nunca se disparó al clickear "Guardar cambios" después — **sin confirmar aún con una 2ª corrida**, criterio habitual del proyecto antes de reportar a devs; (2) `getFilledForm` de Exploración segmentaria y Aparatos y sistemas **siguió en blanco tras los 130s completos de reintentos** en staging (en dev había resuelto ~100s) — no se puede afirmar que sea el mismo retraso transitorio de dev sin evidencia propia de staging (CLAUDE.md §0.4), queda como hallazgo abierto. Pendiente: decidir con Pedro si se corre una 2ª vez para confirmar el hallazgo del diagnóstico, y si se investiga a fondo el retraso de `getFilledForm` en staging (like `_investigar_getfilledform_blanco_dev.js` pero apuntando a staging). Anterior: 2026-07-30, cuarta pasada (**📐 NUEVA NORMA PERMANENTE: metodología de pruebas anti-racionalización, ver `CLAUDE.md` §0.** Se agregó tras un error real: la misma evidencia de un problema (checklists sin cargar en la consulta finalizada) se descartó 3 veces en la misma sesión con una hipótesis benigna sin verificarla. Reglas: inventario completo antes de probar, centinelas automáticos (`auditarPantalla()` en `e2e/utils.js`) en TODA prueba sin importar el foco, verificar todo el estado final y preguntar por otras vistas del mismo dato, prohibido racionalizar anomalías sin evidencia de esa corrida, auditar excepciones ya existentes, lectura estructurada de screenshots. Aplicando la regla de inmediato: se corrigió `consultation.full-flow.spec.js` (el caso de `getFilledForm` en blanco tras Finalizar, antes silenciado como "no bloqueante" sin reintentos reales, ahora reintenta de verdad con esperas largas) y, al correrlo, **el formulario de Exploración segmentaria/Aparatos y sistemas SIGUIÓ EN BLANCO tras 65s completos de reintentos** — ya no era un caso descartable sin más. Investigación de 5 minutos (script `_investigar_getfilledform_blanco_dev.js`) **confirmó que SÍ es transitorio, no pérdida de datos**: en algún punto entre los 65s y ~100s tras Finalizar los datos aparecen completos y se mantienen estables (verificado 5 min seguidos, por API y por UI). La hipótesis vieja era correcta en el fondo — el error fue afirmarla sin evidencia, no la hipótesis en sí. Se subió el reintento del spec a ~130s con holgura real. Ver sección "🔬 Investigación: ¿getFilledForm en blanco es transitorio o real?" más abajo para el detalle completo. Anterior, misma sesión: (**⚠️ RETRACTADO: el hallazgo de "carrera de búsqueda en CIE-10" no se reproduce con timing realista.** A pedido de Pedro, se re-corrió con tecleo humano real (80-140ms/tecla), una pausa de reconsideración de 600ms y espera generosa a que cargara la búsqueda final — el combobox se resolvió correctamente al código final tipeado, y se confirmó vía API que el diagnóstico correcto quedó guardado tras Finalizar. La corrida anterior usaba un hueco de solo 150ms con `fill()` instantáneo, un ritmo no representativo de una persona real — **se retracta el hallazgo, no reportar a devs tal como estaba planteado.** Ver sección "⚠️ Retractado tras re-prueba con timing realista". Anterior, misma sesión: (**🐛 hallazgo NUEVO confirmado con impacto clínico persistente: carrera de búsqueda en el combobox CIE-10 de Diagnóstico** — escribir un código y cambiarlo rápido antes de que resuelva deja seleccionado el código ANTERIOR, no el final tipeado; se completó el flujo real hasta Finalizar Consulta y se confirmó vía API (`getConsultation`) que el diagnóstico equivocado ("TOS" en vez del código buscado) queda guardado PERMANENTEMENTE en la consulta ya finalizada, sin ninguna advertencia — mismo patrón ya conocido en Medicamento/Laboratorio, ahora confirmado también en CIE-10 con persistencia real — ver sección "🐛 Segunda tanda... — 1 hallazgo NUEVO confirmado". El mismo patrón probado en el combobox de Servicios NO reprodujo (filtra localmente, sin ventana de carrera). También se probó sin hallazgos el monto de pago en Ingresos (sobrepago ignorado silenciosamente por el backend — ojo, hay que revisar el body real de la request, no solo el status 2xx) y se dejó sin profundizar la doble reserva de horario en Agenda (requeriría 2 sesiones en paralelo). A pedido de Pedro, esta tanda se enfocó en áreas NUEVAS en vez de reverificar hallazgos ya conocidos, y se evitó deliberadamente el wizard de onboarding "Configuración de tu cuenta" (Pedro lo prueba a mano). Anterior, misma sesión: (**sesión de error guessing en dev buscando 3 clases de bug a pedido de Pedro: valores incorrectos, avance con campos obligatorios vacíos, pérdida de progreso al navegar sin guardar** — ver sección "🎯 Error guessing en DEV — 2026-07-30" más abajo. Resultado: la mayoría de las pantallas probadas (alta de paciente nuevo, ficha del paciente con CP→Colonia/CURP/fecha de nacimiento, signos vitales) están **bien validadas, sin hallazgos nuevos**; sí se **reprodujo 2/2 veces** un 404 de `getFilledForm` justo tras Finalizar una consulta (posible reaparición en dev de un bug ya "solucionado" en staging). También se corrió por primera vez el `consultation.full-flow.spec.js` reescrito el 2026-07-28 (checklist Normal/Anormal + verificación dura post-Finalizar) — la verificación dura pasó limpia las 2 veces. A media sesión Pedro pidió priorizar bugs nuevos sobre reverificar los ya conocidos (memoria `feedback-priorizar-bugs-nuevos-sobre-reverificar`), así que no se insistió en volver a confirmar el bug del botón "Consulta" del 2026-07-28 más allá de 1 corrida adicional (que esta vez, con un paciente distinto, no llegó a navegar a la consulta — variabilidad no investigada). Anterior: 2026-07-28 (**bug real confirmado: el botón "Consulta" (atajo en el perfil del paciente) permite crear la consulta con Tipo de Cita y Hospital vacíos** — a diferencia del wizard completo "Agendar cita" (que sí bloquea "Continuar" si faltan esos campos), el modal del botón "Consulta" deja "Crear consulta" habilitado con los 2 selects en su valor por defecto ("Sin cita"/"Seleccione un hospital") y crea la consulta igual; el error real ("Se necesita asignar el tipo de la consulta") aparece recién más tarde y desconectado, al apretar "Guardar cambios" YA DENTRO de la consulta creada — ver sección "🐛 Bug confirmado: botón 'Consulta' crea con Tipo/Hospital vacíos" más abajo. Se investigó primero la hipótesis del wizard "Agendar cita" como posible vector (descartada: si bloquea correctamente) antes de aislar el botón "Consulta" como el causante real. Mismo día: **`consultation.full-flow.spec.js` endurecido significativamente** — ahora marca y verifica **todos** los checkboxes de Exploración segmentaria/Aparatos y sistemas (no solo 3 al azar), completa el sub-formulario Normal/Anormal + Observaciones que revela cada checkbox (antes no se tocaba), llena también "Otros medicamentos"/"Agrega tratamiento diferente" en Tratamiento, y agrega una **verificación dura post-Finalizar** que relee la consulta completa vía API (`getConsultation`/`getTreatments`/`getForms`/`getFilledForm`) y hace fallar el test si algún campo llenado no coincide exactamente con lo guardado — ver sección "🔧 Endurecimiento de `consultation.full-flow.spec.js`" más abajo (mecanismos descubiertos en el camino: `getFilledForm` puede devolver el formulario en blanco por un rato justo tras Finalizar por retraso de propagación del backend, no pérdida real; hay que usar `page.request` y no `fetch()` de la propia página para leer datos frescos tras finalizar). También se exploró (solo mapeo, sin hallazgo aún) el wizard de **alta de paciente nuevo** (`/Pacientes` → "Agregar paciente" → "Crear perfil de paciente nuevo") — queda como candidato pendiente para probar campos obligatorios/selects dependientes (ver Decisiones abiertas). **Nota:** esta sesión no llegó a ejecutar el full-flow reescrito contra dev todavía — queda pendiente para la próxima corrida confirmar que la nueva verificación dura pasa limpia. Anterior: 2026-07-27 (**corrida completa de la suite Appium de `MediplannerAppiumAutomation/` — app móvil de Mediplanner para PACIENTES** (dependientes, consultas, médicos, medicinas, perfil, bitácora, etc.), a pedido de Pedro — **26 passed, 16 failed, 5 errors** de 42 tests (excluyendo los marcados `destructivo`), en ~26 min contra el emulador ya corriendo. La mayoría de los fallos tiene indicios claros de ser el emulador/infraestructura flaky (ya documentado) y no bugs reales — ver sección "📱 Suite Appium — Mediplanner pacientes" más abajo para el detalle y la lista de candidatos que sí ameritan revisión antes de reportarlos como bugs (el más sospechoso: `test_consultas_reprogramar_cita`, "no hay tipos de consulta disponibles"). Anterior, mismo día: **se intentó reproducir los 2 hallazgos de error guessing en DEV, a pedido de Pedro, para confirmar si son de la plataforma en general o específicos de producción** — resultado mixto: el **Hallazgo 1 (consulta colgada al navegar fuera y volver) SE REPRODUJO igual en dev**, confirmando que es un bug de la plataforma, no de producción específicamente. El **Hallazgo 2 (Finalizar sin re-guardar pierde el cambio) NO se pudo verificar en dev** pese a 6+ intentos — el entorno resultó considerablemente más inestable que producción para este flujo: la cuenta de dev usada cae de forma **intermitente** (a veces sí, a veces no, dentro de la misma sesión ya autenticada) en un wizard extendido de "Configuración de tu cuenta" (6 pasos: perfil/credenciales/consultorios/horarios/tipos de cita/métodos de pago, distinto del onboarding simple ya visto en producción) que bloquea el acceso al Dashboard real de forma no determinística. Se corrigieron varios problemas de timing en el camino (espera robusta de "Cargando...", manejo del wizard de configuración, espera específica de que "Agenda de hoy" termine de poblarse) pero el último bloqueo (el wizard reapareciendo) no se llegó a resolver con más reintentos — se decidió no seguir iterando indefinidamente. Ver sección "🐛 Error guessing — reproducción en DEV" más abajo. Anterior, mismo día: **"error guessing" contra PRODUCCIÓN, a pedido de Pedro: agendar cita nueva e intentar provocar errores reales de médico dentro de la consulta** — 2 hallazgos reales confirmados con evidencia: (1) navegar fuera de una consulta en curso (a Pacientes) y volver por URL directa la deja colgada PERMANENTEMENTE en "Cargando información de consulta" — ni un F5 la recupera, confirmado en 2 corridas independientes; (2) editar un campo (ej. Impresión diagnóstica) DESPUÉS de haber apretado "Guardar cambios" y luego "Finalizar Consulta" sin volver a guardar **pierde ese último cambio en silencio, sin ninguna advertencia**. Ver sección "🐛 Error guessing en PRODUCCIÓN" más abajo para el detalle completo, incluidos 4 escenarios que SÍ pasaron limpio (typo corregido, carrera al cambiar de medicamento buscado, doble-click en "Guardar Respuestas" de checklist, doble-click en "Guardar cambios" global — este último dispara llamadas de red duplicadas pero el backend no duplica los datos). Anterior, mismo día: **prueba manual completa de consulta contra PRODUCCIÓN, con el paciente dedicado "Prueba DE Codigo"** — confirmó que producción YA tiene el nuevo "Modo Completo" (igual que dev desde el 2026-07-23) y que el flujo completo guarda todo correctamente: 0 errores de API en signos vitales + 9 secciones + guardado global (7 endpoints) + finalización, y cada campo llenado coincide exactamente con lo devuelto por la API tras guardar. Ver sección "🟢 Prueba completa en PRODUCCIÓN" más abajo para el detalle, incluidos 2 hallazgos técnicos nuevos (no bugs de la app): los `relacion_id` de formulario de checkboxes son **distintos por entorno** (dev: 116/115, producción: 7/6 — hay que descubrirlos con `getForms` en vez de asumirlos) y `getProceduresList`/`getFilledForm` requieren `doctor_id` en el body además de `paciente_id`/`consulta_id` (si falta, el primero responde error explícito pero el segundo responde 200 con el formulario **vacío en blanco**, sin avisar — riesgo de falso negativo al verificar). Anterior: 2026-07-23 (**segunda ronda de verificación manual, quedó A MEDIAS a pedido de Pedro** — encontró y corrigió otro bug real del test: selección de medicamento/laboratorio en Tratamiento/Laboratorios no comprobaba que la opción clickeada coincidiera con lo buscado, y el log mentía mostrando el término buscado en vez de lo realmente clickeado. Corregido y parcialmente reverificado. Quedó pendiente terminar de verificar el nombre exacto de medicamento/laboratorio guardado y las selecciones de checkboxes de Exploración/Aparatos y sistemas — ver sección "🔍 Segunda ronda de verificación manual". **Hay un script sin terminar:** `_verificar_ultima_consulta.js` en la raíz del repo (no trackeado) — retomar desde ahí, no borrar). Anterior, mismo día: verificación manual post-reescritura encontró y corrigió el bug de "Motivo de consulta" (ver sección "🔍 Verificación manual post-reescritura"). Anterior, mismo día: **`consultation.full-flow.spec.js` reescrito y verificado** para el nuevo "Modo Completo" de la pantalla de Consulta — ver sección "🚨 Rediseño de la pantalla de Consulta" → "✅ Reescritura completada y verificada" más abajo para el detalle completo. Backup de la versión de pestañas en `tests/consultation.full-flow.spec.js.backup`. Anterior, mismo día: hallazgo del rediseño en sí (la pantalla pasó de pestañas clickeables a una sola página con las 10 secciones visibles a la vez), detectado corriendo el full-flow "semi manualmente" apartado por apartado a pedido de Pedro. Anterior: 2026-07-21 (nuevo spec `tests/consultation.user-errors.spec.js` con 6 tests de "error guessing" — errores humanos reales, no fuzzing de seguridad — pero **escrito y sin verificar aún contra dev** (ver sección "🧪 Pendiente: verificar consultation.user-errors.spec.js" más abajo); a pedido de Pedro se dejó documentado como pendiente para retomarlo después, sin correr las 6 corridas todavía). Anterior, mismo día: re-verificación del bug 422 "relacion_id" con 3 corridas limpias de `doctor-consultation` → se baja a "no reproduce", ver sección "🔁 Re-verificación del bug 422". Anterior, mismo día: limpieza de archivos muertos + fix del selector roto de la lista de Pacientes en dev + 2 bugs de test encontrados y corregidos al verificar el fix en los 7 specs afectados (ver sección "🔧 Fix selector de Pacientes" más abajo). Anterior: 2026-07-21 (`8c358b2`, en `AppEstacionamientosColaboradores/` — sesión de reverificación de los bugs de plataforma de Estacionamientos antes de reportarlos formalmente a devs. Resultado: **se retractó el hallazgo de prioridad ALTA "'Cancelar' libera el espacio igual"** — recon manual con coordenadas exactas de los botones y capturas en cada paso probó que la app SÍ se comporta bien; el bug real estaba en el propio harness (`codigos_de_espacios_visibles()` matcheaba el título del sidebar, que queda abierto tras cancelar, como si fuera una fila real de la tabla). Se corrigió el helper (exige contenido multilínea) y se quitó el `xfail` de `test_9_4_liberar_espacio_cancelar`, que ahora pasa limpio. También se **corrigió y acotó** el hallazgo del chip "En línea" (módulo 13): una prueba controlada (verde estable → cortar red real confirmada por `dumpsys` → rojo a los 15s → reconectar → verde de nuevo) mostró que el **color del punto sí refleja la conectividad real** — el bug queda reducido a que el *texto* se queda fijo en "En línea", bajando de prioridad media a cosmético. Se reconfirmaron en vivo (con evidencia nueva, no solo pytest) los 5 hallazgos del portal web (sin evidencia fotográfica obligatoria, duplicidad silenciosa, pérdida de datos sin red, "Levantar falta" sin motivo preseleccionado, copy "obligatorio"/"obligatoria") y los 2 de app móvil que siguen en pie (crash del SDK de Maps confirmado con tombstone nuevo, permiso de ubicación silencioso reconfirmado). Detalle completo en `AppEstacionamientosColaboradores/HALLAZGOS.md`. Anterior: 2026-07-20 (tres commits. Dos en `AppEstacionamientosColaboradores/`, proyecto de estacionamientos "Querétaro con Futuro" — no Mediplanner: `9759a01` completó los módulos 8-13 del checklist de operador en la suite Appium — check-in, espacio ocupado, reporte/infracción, cierre de turno, permisos del sistema y resiliencia/ciclo de vida — más un fix de encoding en `conftest.py`; `f168d89` agregó una nueva suite **Playwright** para el portal web admin del mismo proyecto, con un test combinado Appium+Playwright y 5 hallazgos nuevos de plataforma — detalle completo en el `CONTEXTO.md`/`HALLAZGOS.md` propios de esa carpeta, no duplicado acá. El tercero, `0665923`, es de `MediplannerAppiumAutomation/` — módulo de bitácora + fix de espera en `test_perfil.py`; commiteado directo en este repo porque se confirmó que esa carpeta **ya no tiene su propio `.git`** pese a lo que decía la sección "Repos separados" — ver esa sección, corregida y marcada como pendiente de resolver con Pedro). Anterior: 2026-07-14 (se corrieron en **staging** los mismos tests adaptados en dev el 2026-07-09/10 — `doctor-consultation` y `ingresos` — para confirmar que el porteo, que había quedado sin commitear, funciona; commiteado en `807fe43`/`62e285c`. Ver sección STAGING abajo). Anterior: 2026-07-09 (re-verificación de los 2 bugs de plataforma pendientes: 422 relacion_id sigue vivo con otro endpoint, indicador "sin guardar" de Laboratorios ya no reproduce — ver sección homónima abajo). Mismo día, antes: rediseño de Ingresos + fix del wizard "Agendar cita" + paciente parametrizable. Anterior: 2026-07-07 (verificación de pendientes vs. código + corridas reales; + nueva suite Appium independiente en `AppEstacionamientosColaboradores/`)

---

## 🔍 Corrida completa de la suite dev + observación metodológica — 2026-08-17

Pedro pidió correr pruebas en dev buscando cambios y errores, y que se observara **cada paso** de las automatizaciones (no solo si terminaban en verde), aplicando la metodología de `CLAUDE.md` §0 de punta a punta. Alcance: los 12 specs de `tests/` con asserts duros (`system-health`, `dashboard`, `ajustes-servicios`, `appointments-create`, `reportes`, `subir-estudios`, `recetas`, `ingresos`, `consultation-inputs-validation`, `consultation-user-errors`, `doctor-consultation`, `vacunacion-ciclo-completo`), uno por uno, revisando el log completo de cada corrida (errores de consola, responses de API, screenshots) antes de pasar al siguiente. Se dejaron fuera a propósito los `*.explorar.*` (sin asserts, son mapeo) y los `stress-*` (mucho más largos, no pedidos esta vez).

### 🐛 Hallazgo 1 — CSP de dev bloquea a Microsoft Clarity (reproducido 3/3)

`system-health.spec.ts` falló de forma 100% determinista (2/2 corridas idénticas + confirmado con un script de diagnóstico aparte) por 2 console errors de Content-Security-Policy: la política de `admin-dev.mediplanner.mx` solo permite `https://www.clarity.ms` en `script-src`/`img-src`, pero el propio sitio intenta cargar Clarity desde `https://scripts.clarity.ms/0.8.69/clarity.js` (script) y `https://c.clarity.ms/c.gif` (píxel) — subdominios distintos, bloqueados. Un tercer error relacionado apareció una vez en `ajustes.servicios.spec.ts`: `connect-src` tampoco incluye `www.google.com`, usado por un fallback de Google Analytics.

**Impacto:** no afecta funcionalidad clínica — el contenido de la página se verificó limpio (sin `undefined`/`NaN`/`null`, sin requests con status ≥400). El efecto real es que **Microsoft Clarity nunca recolecta analítica/session-replay en dev** (falla en silencio, sin avisar a nadie). `reportes.spec.ts` y otros specs "Alta confianza" ya filtran este ruido explícitamente (`EXTERNAL_NOISE` regex incluye `clarity\.ms`), así que no los hace fallar — el problema es específico de `system-health.spec.ts`, que hace `expect(consoleErrors).toEqual([])` sin ningún filtro.

**Dato curioso de paso:** revisando el historial de git, `tests/system-health.spec.ts` no se había tocado desde el commit inicial del repo — el "fix" de `networkidle`→`load` y `/dashboard`→`/Dashboard` documentado el 2026-08-03 se aplicó solo a la copia de **staging**, nunca se portó de vuelta a dev. Es decir, esta podría ser la primera vez que se corre este spec contra dev con el resultado realmente examinado.

Script de diagnóstico: `_diagnostico_csp_clarity_dev.js`.

### 🐛 Hallazgo 2 — bug de Servicios confirmado también en dev (no solo staging)

El bug documentado el 2026-08-03 en staging (`saveService` responde 200 con un `id_servicio` nuevo pero el servicio nunca aparece después en `getServices`) **reprodujo igual en dev**: `saveService` → 200 `{"id_servicio":12,"message":"Servicio guardado correctamente"}`, pero la siguiente `getServices` (sin caché) solo devolvió los servicios preexistentes — `"QA_TEST_SERVICIO_NO_BORRAR_..."` no apareció. La API de Servicios en sí no devolvió ningún error (0 fallas). Con esta 2ª confirmación **en un entorno completamente distinto**, sube la confianza de que es un bug real de backend/base de datos, no una particularidad de staging — más fuerte que pedir una 2ª corrida en el mismo entorno.

### 🐛🔴 Hallazgo 3 (severidad ALTA) — signos vitales con valores clínicamente imposibles se guardan sin ninguna validación

`consultation.inputs-validation.spec.ts` encontró que, al tipear valores extremos, **6 de 14 campos de "Capturar signos vitales" retienen el valor tal cual, sin filtrar ni recortar**: Peso ("99999"), Talla ("999"), Presión Arterial ("999/999"), Frecuencia Cardiaca ("999"), Oxigenación ("500"), Frecuencia Respiratoria ("200"). Solo Temperatura mostró algún tipo de tope funcional (recortó "150"→"15"). A diferencia de los valores NO numéricos (letras — correctamente filtrados en todos los campos) y de los valores NEGATIVOS (ya confirmados bloqueados el 2026-07-30, con "Guardar" deshabilitado), **los valores positivos absurdos no tienen ningún límite superior**.

El test original solo mide el `<input>` en sí (no si se puede guardar), así que se verificó el siguiente paso con un script dedicado (`_verificar_signos_vitales_extremos_dev.js`): se llenaron todos los campos con valores válidos salvo Frecuencia Cardiaca = 999, y **el botón "Guardar" se habilitó igual, y `POST /api/consultations/registerVitalSigns` respondió `200 {"message":"Signos vitales registrados correctamente","consulta_id":6}`**. Un pulso de 999 latidos por minuto (fisiológicamente imposible) quedó guardado de forma permanente en el expediente de un paciente de prueba, sin ninguna advertencia en ningún punto del flujo. Es razonable esperar que el mismo problema aplique a Peso/Talla/Presión/Oxigenación/Frecuencia Respiratoria dado que sus `<input>` tienen el mismo comportamiento sin filtro — no se probó cada uno guardando por separado, pero el mecanismo (ausencia de validación de rango) es el mismo.

**Candidato fuerte para reportar formalmente a devs** — a diferencia de otros hallazgos "silenciosos" (datos que no aparecen), este es un caso de **datos incorrectos que SÍ quedan visibles y accesibles en el expediente**, con riesgo directo de mala interpretación clínica.

### 🐛 Hallazgo 4 — regresión en Vacunación: borrar todas las dosis ya no persiste

`vacunacion.ciclo-completo.spec.ts` (destructivo, sobre "Agustin Tapia") falló en el paso "VERIFICAR vacío (tras refrescar)": tras clickear las 32 "×" de borrado de dosis (que sí desaparecieron de la UI, assert intermedio pasó) y buscar el botón "Guardar cambios" para persistir el borrado en batch, **el botón no apareció** (`saveVaccinesUser` de "Guardar": 0 llamadas) — el borrado nunca llegó a enviarse al backend. Al navegar de nuevo a Vacunación, las 32 dosis originales seguían intactas.

Esto es una diferencia real de comportamiento respecto a la última corrida documentada (2026-07-21): en esa ocasión el mismo síntoma ("no aparece 'Guardar cambios' tras borrar dosis") ya se había notado, pero se catalogó como **hallazgo blando sin severidad** porque el resultado final tras refrescar SÍ daba 0 dosis (el borrado persistía por algún otro mecanismo). Ahora el resultado final es 32, no 0 — el mismo síntoma cosmético ahora sí bloquea la funcionalidad. **El dato del paciente de prueba no quedó corrupto** (no hubo borrado parcial, solo ningún cambio) — el test se detuvo ahí y no llegó a re-registrar las dosis.

**Nota metodológica:** este hallazgo, junto con el de Servicios, forma un patrón: dos módulos distintos (Servicios, Vacunación) donde una operación de escritura "parece" exitosa en la UI pero no se refleja tras releer los datos — vale la pena tenerlo en cuenta si aparece un tercer caso similar.

### ⚠️ Hallazgo abierto, sin confirmar — Detalle de ingreso atascado en skeleton

Durante `ingresos.spec.ts`, tras abrir un ingreso "pendiente" la pantalla "Detalle de ingreso" se vio **completamente en estado skeleton** (los 3 resúmenes, "Cargos" y "Pagos", todo en placeholders grises, nada renderizó) en la captura final, coincidiendo con 2 llamadas a `getFiscalData` con `net::ERR_ABORTED`. El propio test ya esperaba 12s por este escenario (con un comentario citando una investigación previa del 2026-07-14 donde tardó hasta 8s en asentarse) y aun así no alcanzó. Se intentó reproducir en vivo con una espera de hasta 60s, pero **el ingreso pendiente específico ya no estaba en la lista de "Pendiente"** minutos después (dev es un entorno compartido, los datos cambiaron entre corridas) — no se pudo confirmar si hubiera resuelto con más tiempo o si es un colgado real. Podría ser el mismo patrón de lentitud ya conocido, o una reaparición parcial del bug `DetallePagos` (TypeError con `getFiscalData` vacío) que se dio por resuelto el 2026-07-21. **Queda como hallazgo abierto** — si se repite, valdría un script de monitoreo dedicado (mismo patrón que `_investigar_getfilledform_blanco_dev.js`) que abra el detalle y monitoree varios minutos.

### 🔧 Mantenimiento de tests aplicado en vivo (selectores desactualizados, confirmados contra el DOM real)

- **`tests/reportes.spec.ts`** (4 fixes, ahora pasa 2/2 limpio): label de KPI "Número de consultas" → "Citas del período" (cambio de copy); opción de rango de fechas "Últimos mes" → "Último mes"; faltaba un clic explícito en "Buscar" tras cambiar el select para que el panel de filtros aplicara el cambio (confirmado en vivo: sin ese clic, `getDashboardPayments` nunca se disparaba); regex de "Período" corregido (no hay espacio entre la etiqueta y las fechas en el DOM real: `"Período10-08-2026 – 17-08-2026"`); opción de Estatus "Todos los estatus" → "Todos".
- **`tests/subir-estudios.spec.ts`** (2 fixes): nombre de paciente en la tabla `a.font-semibold` → `span.font-semibold` (mismo cambio ya documentado en otros 7 specs el 2026-07-21, pero este archivo había quedado afuera); tab "Consultas" del perfil del paciente `a:has-text(...)` → también acepta `button:has-text(...)` (el tab ahora es un `<button>`, no un `<a>`). Antes de estos fixes, el test no podía ni seleccionar un paciente — con ellos, al menos avanza hasta encontrar (o no) consultas reales del paciente elegido al azar.

### 📋 Gaps de cobertura documentados (no son bugs de la app)

- **`appointments.create.spec.ts`** (2do test, "Confirm scheduled appointment from agenda"): 0 asserts duros después de la navegación inicial; busca el texto literal "agendada"/"programada" en la Agenda, que no existe en ningún lado de la UI real (que muestra el título de la cita, ej. "Consulta inicial", bajo un toggle que alterna entre vista "Mostrando citas"/"Mostrando consultas"). Se verificó en vivo que la cita SÍ se crea y se muestra correctamente (`getFilteredAppointments` la devuelve, el texto "12:15" es visible) — el test simplemente no puede detectarlo y siempre reporta "passed" pase lo que pase.
- **`subir-estudios.spec.ts`**: incluso tras los 2 fixes de selectores, el archivo sigue sin ningún assert duro sobre el flujo real de subir un estudio (todo es `.catch(() => false)` + `console.log` de warning) — un "passed" no prueba que la subida de resultados de laboratorio funcione, solo que el script no crasheó.
- **`consultation.user-errors.spec.js`**: 5 de 6 tests fallaron todos en el mismo punto — el helper compartido `iniciarConsultaDeHoy` solo llena Peso y Talla en el modal de signos vitales (2 de ~7 campos), dejando "Guardar" deshabilitado. Esto es consistente con lo ya documentado (este archivo sigue con el modelo pre-rediseño "Modo Completo" del 23-jul, nunca actualizado) — no es un bug nuevo de la plataforma, es la reescritura pendiente ya identificada.

### ✅ Reconfirmación (no es hallazgo nuevo) — `doctor-consultation` (full-flow)

La verificación dura post-Finalizar pasó limpio en **10 de 12 campos** (motivo, padecimiento, notas de evolución, nombre referido, apariencia, impresión diagnóstica, indicaciones general/procedimiento, diagnósticos, servicios, medicamentos — todos coinciden exactamente con lo llenado). Los únicos 2 fallos fueron el ya documentado 404 de `getFilledForm` para Exploración segmentaria/Aparatos y sistemas tras Finalizar, con un timing (+95.67s) casi idéntico al de la sesión del 2026-07-30 — sigue sin resolverse, y esta vez los 130s de reintentos tampoco alcanzaron.

**Scripts de investigación nuevos** (raíz, no trackeados): `_diagnostico_csp_clarity_dev.js`, `_diagnostico_agenda_toggle_dev.js` (usado para descartar un falso hallazgo en Agenda — ver nota abajo), `_investigar_detalle_ingreso_skeleton_dev.js`, `_verificar_signos_vitales_extremos_dev.js`.

**Nota — falsa alarma descartada con evidencia:** al revisar `appointments.create.spec.ts`, un toggle "Mostrando citas" que parecía apagado hizo sospechar de un bug de visibilidad de citas — investigado en vivo, resultó ser un selector de vista (citas pendientes vs. consultas ya atendidas) funcionando correctamente, no un bug. Documentado en el gap de cobertura de arriba, no como hallazgo de plataforma.

---

## 🔧 Mejora de detección de errores — Suite Appium — 2026-08-03

Pedro pidió analizar la suite Appium de `MediplannerAppiumAutomation/Appium/` (app móvil de pacientes) para identificar: (a) tests que prueban lo mismo o muy parecido, y (b) qué tan bien detectan errores reales de verdad (no solo si pasan corriendo). Del análisis salieron 3 acciones acordadas con Pedro: auditar `consultas_page.py` (la parte de la suite con mayor confianza declarada), reforzar 3 tests con verificación de persistencia real, y eliminar 2 tests redundantes.

### ✅ Auditoría de `consultas_page.py`: confirmada sólida

Los métodos `confirmar_asistencia()`, `reprogramar()` y `agendar_cita()` sí salen a "Programadas" y releen el estado real (comparando `content-desc` antes/después) en vez de confiar en el DOM en memoria justo tras el click — el código coincide exactamente con lo que sus comentarios prometen. No se necesitó ningún cambio ahí.

### 🐛 2 bugs reales de la suite encontrados (no de la app) al reforzar 3 tests

Al agregar verificación de persistencia real a `test_home_medicamento_no_tomado`, `test_medicamento.py::test_medicamento_registro_de_toma` y `test_dependientes.py::test_dependientes_agregar` (reabrir la pantalla/drawer y confirmar que el dato realmente quedó guardado, en vez de solo comprobar que un modal se cerró), las primeras corridas fallaron — investigado con evidencia real (capturas del emulador cada 1 segundo + log crudo de Appium, no solo el log de pytest) antes de aceptar cualquier hipótesis, siguiendo la norma de `CLAUDE.md` §0.4:

1. **`ocultar_keyboard()` (en `pages/base_page.py`) manda KEYCODE_ESCAPE, que cierra modales enteros, no solo el teclado.** Confirmado en el log crudo de Appium: `mobile: hideKeyboard` ejecuta `adb shell input keyevent 111` (KEYCODE_ESCAPE). Con capturas cada 1s se vio que, justo después de teclear un comentario en el modal de "No Tomado", el modal se cerraba SOLO (sin haber tocado "Aceptar"), descartando el comentario — la app trata ESCAPE como "cancelar". El botón de confirmación ya estaba completamente visible sin necesidad de ocultar el teclado en los 3 casos, así que se quitó la llamada (innecesaria y destructiva) de `test_home.py`, `test_medicamento.py` y `test_dependientes.py` (este último la tenía justo antes de buscar "Continuar").
2. **`test_dependientes.py::_llenar_formulario()` no llamaba `.clear()` antes de `.send_keys()`.** Como el proceso de la app no se reinicia entre corridas de pytest (sesiones Appium sucesivas reutilizan el mismo estado de la app), el texto de una corrida anterior fallida quedaba en los campos y se concatenaba con el de la corrida nueva — confirmado viendo en capturas consecutivas cómo "Nombre" pasaba de `"Pedro"` a `"PedroLaura"` (con "Laura" siendo el nombre random de la nueva corrida) y "Apellido paterno"/"materno" de `"Garcia"`/`"Perez"` a `"GarciaGarcia"`/`"PerezPerez"` (son literales fijos en el test, no random). Los datos resultantes eran inválidos y "Continuar" nunca se habilitaba — no era un bug de la app. Se agregó `.clear()` a los 3 campos.

Tras corregir ambos, `test_dependientes_agregar` reveló además un problema de datos (no de código): la cuenta de prueba llegó a su **límite de dependientes**, lo que hacía fallar el alta con un mensaje distinto. Pedro liberó 2 cupos borrando dependientes manualmente y la corrida siguiente pasó limpia.

### ✅ `test_dependientes_desvincular` reescrito con el flujo real (Pedro identificó el flujo correcto tras revisar la app a mano)

La primera versión reforzada de este test asumía (mal) que "Desvincular" está en el perfil del dependiente visto desde el drawer del titular — tocar el dependiente ahí en realidad muestra el perfil PROPIO del titular, no el del dependiente (esto coincidía con la cautela ya documentada en memoria `feedback-appium-no-cambiar-perfil-drawer`). Pedro aportó el flujo real de la app, confirmado con capturas y reescrito en el test:

1. Drawer del titular → click en el dependiente → **cambia de contexto**: ahora toda la app (Inicio, Perfil, etc.) refleja al dependiente.
2. En ese contexto: Perfil → Compartir → el titular aparece listado como co-titular compartido → click en su nombre.
3. Se abre la pantalla "Compartiendo" (nombre del titular + toggles de permisos + botón "Desvincular"). Click en "Desvincular".
4. Volver a Inicio, abrir el drawer, click en el titular (fila "Mi Perfil") para volver a SU contexto, y confirmar que el dependiente ya no aparece en "Mis Perfiles".

Al implementar este flujo se descubrieron y corrigieron 2 problemas más, ambos de timing (no del test en sí ni bugs de la app):
- El **cambio de contexto de vuelta al titular no es instantáneo** (aparece un spinner de carga) — confirmado con capturas cada 1s: el primer intento de reabrir el drawer justo después del click seguía mostrando el contexto del dependiente. Se agregó un reintento (hasta 6 veces, con esperas de 2s) que confirma la fila "Mi Perfil" con el nombre del TITULAR antes de continuar.
- La **desvinculación en sí puede tardar unos segundos en propagarse** al listado "Mis Perfiles" — mismo patrón de retraso transitorio de backend ya documentado en otras partes del proyecto (`getFilledForm` tras Finalizar). Se agregaron reintentos con esperas reales (5s/10s/15s) antes de concluir que no se persistió; en la corrida que lo confirmó, se resolvió con el primer reintento de 5s.

**Resultado: `test_dependientes_desvincular` pasa limpio.** Verificado 2 veces (una vez desvinculando a "Miguel Garcia Perez", otra a "Kenia Garcia Perez", ambas creadas en esta misma sesión) con persistencia real confirmada.

### 🗑️ 2 tests eliminados por redundancia (sin pérdida de cobertura)

- **`test_doctor_search`** (`test_basico.py`) — subsumido por completo por `test_medicos_flujo_completo` (`test_medicos_filtros.py`), que cubre lista + filtro de texto + filtro por estado con los mismos asserts y más.
- **`test_bitacora_smoke_categoria`** (`test_bitacora.py`, parametrizado sobre las 8 categorías del modal) — las 8 categorías ya tienen, cada una, un test dedicado en el mismo archivo que verifica habilitación/deshabilitación de botones **y** persistencia real tras guardar; el smoke test solo comprobaba que el modal se cerraba al elegir la categoría, un subconjunto estricto de lo que ya cubrían los tests dedicados.

### 📊 Antes / Después

| Test | Antes | Después |
|---|---|---|
| `test_home_medicamento_no_tomado` | ❌ FAILED — `ocultar_keyboard()` cerraba el modal, "Aceptar" nunca aparecía | ✅ PASSED — persistencia verificada (o informa "ya registrado hoy" si aplica) |
| `test_medicamento_registro_de_toma` | ❌ FAILED — mismo bug de `ocultar_keyboard()` | ✅ PASSED — persistencia verificada (esta corrida tomó la rama informativa, medicamento ya registrado) |
| `test_dependientes_agregar` | ❌ FAILED — texto concatenado entre corridas, "Continuar" nunca se habilitaba | ✅ PASSED (tras liberar cupos de dependientes) |
| `test_dependientes_desvincular` | ❌ FAILED — "Desvincular" no aparecía (flujo asumido incorrecto) | ✅ PASSED — reescrito con el flujo real (cambiar de contexto → Compartir del dependiente → Desvincular al titular), con reintentos de timing para el cambio de contexto y la propagación del backend |
| **Total (`test_home.py`+`test_medicamento.py`+`test_dependientes.py`)** | **4 failed, 202s** | **4 passed** |
| `test_doctor_search` | Existía, redundante | Eliminado |
| `test_bitacora_smoke_categoria` | Existía, redundante | Eliminado |

---

## 🧪 Porteo a STAGING — 2026-08-03

Pedro pidió analizar los 19 specs de `tests/` (dev) y clasificarlos por solidez antes de portar a staging los que valieran la pena. Se armó una tabla Archivo | Qué prueba | Confianza (Alta/Media/Baja) con razones concretas (catches silenciosos que ocultan bugs, si verifican UI+API o solo navegan, dependencia de datos hardcodeados). Resultado: 7 "Alta" (`ajustes.servicios`, `auth.setup`, `consultation.full-flow`, `dashboard`, `recetas`, `reportes`, `vacunacion.ciclo-completo`), 2 "Media" (`ingresos`, `system-health`) que no dependen del modelo de pestañas obsoleto de Consulta — se excluyó `consultation.user-errors.spec.js` (Media pero sigue con pestañas viejas, sin portar al rediseño del 23-jul) e `inputs-validation`/`explorar.*` (Baja, sin asserts duros o exploratorios por diseño).

**De esa lista, en staging ya existían** `auth.setup`, `consultation.full-flow`, `ingresos`, `system-health`, `vacunacion-ciclo-completo` (y `appointments-create`, `percentil-explorar`, `subir-estudios`, `vacunacion-explorar`, no en la lista Alta/Media pero ya portados de antes). **Faltaban por portar:** `ajustes.servicios.spec.ts`, `dashboard.spec.js`, `recetas.spec.ts` — se copiaron a `Tests_Staging/` y se agregaron sus proyectos (`ajustes-servicios`, `dashboard`, `recetas`) a `Mediplanner Staging/playwright.config.js`.

### 1ª corrida completa (14 tests): 5 pasan, 9 fallan

Pasan: `auth.setup`, `appointments-create` (×2), `ingresos` (×2). Fallan: `percentil-explorar`, `doctor-consultation`, `system-health`, `subir-estudios`, `vacunacion-explorar`, `vacunacion-ciclo-completo`, `ajustes-servicios`, `dashboard`, `recetas` — todos con timeouts esperando selectores o `networkidle`, **reproducidos idénticos en una 2ª corrida completa** (descartada la flakiness como explicación única).

Pedro pidió excluir de esta ronda el hallazgo de `doctor-consultation` (checklists en blanco tras Finalizar, ya documentado el 2026-07-31) y, para el resto, distinguir explícitamente selectores/rutas desactualizadas (adaptar el test) de bugs reales de la plataforma (no adaptar, reportar).

### Investigación en vivo contra staging (Playwright directo, no MCP) — 7 de 9 eran selectores desactualizados

- **`percentil-explorar`:** el placeholder real del buscador de pacientes es `"Buscar paciente"`, no `"Buscar Usuarios"` (el de dev). Corregido.
- **`vacunacion-explorar` y `vacunacion-ciclo-completo`:** el nombre del paciente en la lista vive en un `<span class="font-semibold text-sm text-gray-900">`, no en un `<a>` — confirmado inspeccionando el DOM real. Corregido el selector en ambos specs (`goToVacunacion`).
- **`system-health` y `subir-estudios`:** usaban `page.waitForLoadState('networkidle')`, que nunca se cumple en este entorno (tráfico constante de GA/Zendesk/Clarity, ya documentado en `e2e/utils.js`). Reemplazado por `'load'` + espera, mismo patrón que ya usa el resto del repo (`system-health` también corregido de `/dashboard` a `/Dashboard`, mayúscula).
- **`recetas.spec.ts`:** el paciente hardcodeado `"Agustin Tapia"` **no existe en staging** — confirmado contra la API real (`getPatients`, 18 pacientes totales, ninguno con ese nombre; es exclusivo de dev). Se investigó cuál paciente real de staging tiene recetas cargadas (`Pedro Quijada Anaya`: 7 recetas/1 página; `Percentil Prueba Prueba`: 21 recetas/3 páginas; `Joel Moreno Ayala`: 17/2 páginas) y se reemplazó por **`Percentil Prueba Prueba`** — mejor candidato porque el test también verifica paginación real ("Siguiente" cambia el rango mostrado), algo que "Agustin Tapia" nunca hubiera podido ejercitar.

Los 6 specs de arriba, tras el fix, **pasan y se re-confirmaron con una corrida adicional** (recetas: paginación real 1–10 → 11–20 de 21, confirmado).

### 🐛 Bug real confirmado (no es error del test): `ajustes.servicios.spec.ts` — el servicio creado nunca aparece en `getServices`

Antes de aceptar el fallo como bug de plataforma se revisó paso a paso la lógica del propio test (a pedido de Pedro) para descartar que fuera un error de test disfrazado:
- El filtro "Sólo activos" **no es la causa**: confirmado que filtra solo del lado del cliente (no dispara una nueva llamada a `getServices` al togglear) y que la API **sí** devuelve servicios inactivos normalmente (un servicio viejo con `activo:false` aparece sin problema en la respuesta).
- Se reprodujo el flujo completo de forma independiente, fuera del test: `saveService` responde 200 `"Servicio guardado correctamente"` con un `id_servicio` nuevo e incremental (probado con ids 5 y 6 en corridas distintas) — pero la siguiente llamada a `getServices` (navegación fresca, sin caché) **sigue devolviendo únicamente los mismos 3 servicios de siempre** (ids 2, 3, 4). El servicio nuevo no aparece bajo ningún id.

**Conclusión: el test está haciendo los pasos correctos — es un bug real y reproducible de la plataforma** (guardado exitoso reportado pero no persistido/indexado para lectura). Documentado como hallazgo abierto, no reportado aún formalmente a devs (falta decidir con Pedro si amerita una 2ª confirmación independiente antes, siguiendo el criterio habitual del proyecto).

### ⚠️ Hallazgo abierto: cuenta de staging atascada en el wizard de onboarding — afecta `dashboard.spec.js`

Investigando por qué `dashboard.spec.js` seguía fallando (tarjeta "CONSULTAS" sin contenido) incluso tras agregarle el mismo salto de wizard que ya usa `consultation.full-flow.spec.js`, se encontró algo más raro que un selector roto: la cuenta `dr@rym-solutions.com` está detenida en el **Paso 3 de 6** ("Consultorios") del wizard "Configuración de tu cuenta", y **la SPA rebota de vuelta a `/wizard/onboarding` unos segundos después** de hacer clic en "Configurar más tarde" — a veces alcanza a mostrar el Dashboard real el tiempo suficiente para que las llamadas a la API respondan (confirmado: `getDashboardData` con `consultas=2` reales), pero no siempre el tiempo suficiente para que la UI se estabilice y se pueda leer la tarjeta KPI. El widget de chat de Zendesk además tapa visualmente el link "Configurar más tarde" en pantalla, lo que puede estar afectando el clic. **No se investigó más a fondo ni se intentó completar el wizard** — por instrucción existente (`[[feedback-no-probar-wizard-configuracion-cuenta]]`), Pedro prueba el wizard a mano. Queda pendiente que Pedro complete el wizard manualmente en staging para esta cuenta; hasta entonces `dashboard.spec.js` puede seguir siendo flaky en ese punto específico.

**Estado final de la corrida (excluyendo `doctor-consultation`, fuera de alcance por pedido de Pedro):** 12/14 pasan. Los únicos 2 sin resolver son los 2 hallazgos de arriba (bug real de Servicios + wizard atascado), ambos documentados como abiertos, no como fallos de test.

---

## 🧪 Porteo a STAGING — 2026-07-31

Pedro pidió correr en staging las mismas automatizaciones en las que se venía trabajando en dev (el `consultation.full-flow.spec.js` endurecido con checklists Normal/Anormal + verificación dura post-Finalizar, y la metodología anti-racionalización de `CLAUDE.md` §0).

**Estado previo de staging, encontrado antes de correr nada:** `Mediplanner Staging/Tests_Staging/consultation.full-flow.spec.js` era la versión VIEJA (modelo de pestañas, última tocada 2026-07-09), de antes del rediseño "Modo Completo" del 2026-07-23 en dev — no se había vuelto a portar desde entonces. `Mediplanner Staging/e2e/utils.js` tampoco tenía `auditarPantalla()` (agregado en dev el 2026-07-30, ver CLAUDE.md §0.2).

**Porteo aplicado:**
1. Se agregó `auditarPantalla()` a `Mediplanner Staging/e2e/utils.js` (idéntica a la de dev, solo se ajustó el comentario de contexto).
2. Se respaldó el spec viejo de staging como `consultation.full-flow.spec.js.backup` y se reemplazó por la versión endurecida completa de dev (Modo Completo + checklists ítem-por-ítem + verificación dura post-Finalizar vía API, reintentos de ~130s para `getFilledForm`). El resto de `e2e/utils.js` de staging (createAppointment, navegación de calendario, etc.) NO se tocó — ya tenía su propia lógica adaptada a las particularidades de staging de sesiones anteriores (2026-07-09/14) y el spec la reutiliza tal cual vía `require('../e2e/utils.js')`.

**Primera corrida contra staging (paciente "Percentil Prueba Prueba", `consulta_id=8`, `paciente_id=1025`) — FALLÓ, con 4 inconsistencias reales:**

1. **🐛 Posible bug nuevo (sin confirmar aún — falta 2ª corrida):** `impresion_diagnostico` quedó **vacío** (`""`) y `diagnosticos` quedó **vacío** (0 elementos) tras Finalizar, pese a que el log de la corrida muestra que el CIE-10 sí se seleccionó ("A09X - DIARREA...") y la Impresión diagnóstica sí se tipeó. Se observó en el log de red: seleccionar el CIE-10 dispara un `POST /api/consultations/editConsultation` **automático e inmediato** (antes incluso de que terminara de tipearse la Impresión diagnóstica), que respondió 200 "Consulta actualizada exitosamente". Después, al clickear el botón global "Guardar cambios" (~15s más tarde), las llamadas disparadas fueron solo `setTreatments`, `setFreeTreatmentsConsultation`, `setProceduresConsultation`, `addServices`, `addNote` — **`addDiagnosis` nunca se disparó**. Hipótesis a confirmar: el auto-guardado inmediato al seleccionar CIE-10 podría estar reseteando el estado "sin guardar" de la sección Diagnóstico completa, haciendo que el guardado global posterior ya no la detecte como pendiente — y el diagnóstico + impresión diagnóstica tecleados después se pierden en silencio. **No se reporta todavía como bug confirmado** (criterio habitual del proyecto: 2-3 corridas limpias antes de descartar azar, ver `CLAUDE.md` §0.4) — podría ser una condición de carrera específica de esta corrida. Pendiente: repetir la corrida y, si se repite, aislarlo con un script dedicado (seleccionar CIE-10, esperar a que el auto-guardado responda, recién ahí seguir llenando el resto, y comparar contra hacerlo todo rápido sin esperar).
2. **⚠️ Hallazgo abierto, no descartado:** `getFilledForm` de **Exploración segmentaria** y **Aparatos y sistemas** siguió devolviendo el formulario en blanco tras los **130 segundos completos** de reintentos (todos los reintentos agotados). En dev (2026-07-30) el mismo mecanismo había resuelto en algún punto entre 65-100s. **No se asume que el comportamiento de dev aplique igual en staging** (sería exactamente la racionalización que `CLAUDE.md` §0.4 prohíbe) — queda como hallazgo abierto. Si se quiere confirmar si es el mismo retraso transitorio (solo que más largo en staging) o algo distinto, habría que correr un script de monitoreo real tipo `_investigar_getfilledform_blanco_dev.js` pero apuntando a staging, seguido varios minutos después de Finalizar.
3. Los 2 puntos anteriores en conjunto hicieron fallar el test: `expect(inconsistencias).toEqual([])` y `expect(result.failedApiCalls.length).toBe(0)` (2 responses 404 de `getFilledForm` justo tras Finalizar, mismo patrón ya conocido).
4. **Todo lo demás pasó correcto:** motivo, padecimiento, notas de evolución, nombre referido, apariencia, indicaciones de tratamiento/laboratorio, servicios (1), medicamentos (2) — coincidieron exactamente con lo llenado.

Log completo de la corrida: `test-results-staging-fullflow.log` en la raíz del repo (no trackeado).

**Pendiente, a decidir con Pedro:** ¿correr una 2ª vez para confirmar/descartar el hallazgo del diagnóstico perdido? ¿investigar a fondo el retraso de `getFilledForm` en staging? Por ahora ninguno de los 2 se reporta como bug confirmado a devs.

### ✅ 2ª corrida (misma sesión) — diagnóstico/impresión diagnóstica: NO reprodujo, se retracta ese hallazgo

A pedido de Pedro, se dejó de lado `getFilledForm` por ahora y se corrió una 2ª vez el mismo spec contra staging para verificar la solidez del registro de datos de la consulta (paciente "Percentil Prueba Prueba", `consulta_id=9`).

**Resultado: TODOS los campos de la consulta (motivo, padecimiento, notas_evolución, nombre_referido, apariencia, impresión_diagnóstica, indicaciones_general, indicaciones_procedimiento, diagnósticos, servicios, medicamentos) coincidieron exactamente con lo llenado.** A diferencia de la 1ª corrida, esta vez el clic en "Guardar cambios" global **sí disparó `addDiagnosis`** (200 OK, junto con `setTreatments`/`setFreeTreatmentsConsultation`/`setProceduresConsultation`/`addServices`/`addNote`) y `getConsultation` post-Finalizar confirmó `diagnosticos: 1 elemento` e `impresion_diagnostico` con el texto exacto.

**Conclusión: se retracta el hallazgo #1 (diagnóstico/impresión diagnóstica perdidos) — no reprodujo.** Fue evidentemente una condición de carrera puntual de la 1ª corrida (el auto-`editConsultation` disparado al seleccionar CIE-10 coincidiendo mal con el guardado global), no un bug determinístico. **No se reporta a devs.** El único motivo de fallo de esta 2ª corrida fue el hallazgo #2 (`getFilledForm` en blanco tras 130s), que por pedido explícito de Pedro se dejó fuera del alcance de esta verificación — sigue abierto, sin investigar a fondo todavía.

**Veredicto sobre la solidez del registro de datos de la consulta en staging (excluyendo `getFilledForm`): sólido — 2/2 corridas con todos los campos verificados correctos.**

### 🔬 Investigación del `getFilledForm` en blanco — STAGING (misma sesión)

A pedido de Pedro, se investigó el hallazgo #2 (ignorado hasta este punto). Se porteó `_investigar_getfilledform_blanco_dev.js` → `Mediplanner Staging/_investigar_getfilledform_blanco_staging.js`, apuntando a la consulta ya finalizada de la 2ª corrida (`consulta_id=9`, `paciente_id=1025`, "Percentil Prueba Prueba"). Dos intentos fallaron por no capturar el token de sesión a tiempo (storageState vencido / carrera al capturar el header) — se corrigió con reintentos explícitos y una re-autenticación (`--project=setup`) antes de la corrida que sí funcionó.

**Resultado: para cuando arrancó este monitoreo (bastante después de Finalizar — hubo varios minutos de por medio con el análisis de la corrida anterior, la corrección del script y los 2 intentos fallidos), Exploración segmentaria (17/17 elementos) y Aparatos y sistemas (31/31 elementos) YA mostraban los datos completos, y se mantuvieron estables sin volver a vaciarse durante los 343s (5.7 min) completos de monitoreo, con 0 apariciones de "Cargando preguntas" en la UI.**

**Conclusión: se confirma que en STAGING también es un retraso de propagación transitorio, NO pérdida de datos** — mismo patrón que en dev, pero el margen de espera necesario en staging parece ser mayor a los ~130s que el spec reintenta actualmente (en la 2ª corrida del full-flow seguía en blanco a los 130s). No se pudo acotar el momento exacto de la resolución esta vez (a diferencia de la investigación de dev, que sí lo acotó entre 65-100s) porque pasó demasiado tiempo entre Finalizar y el inicio de este monitoreo. **Si se quiere acotar el margen real en staging, habría que repetir con un monitoreo que arranque inmediatamente después de Finalizar** (como se hizo en dev). Por ahora, con esta evidencia, no hace falta subir el margen de reintentos del spec de staging todavía — no hay caso confirmado de que se quede en blanco permanentemente.
...
### ✅ Resultado de la investigación (2026-07-30, misma sesión): CONFIRMADO transitorio, no es pérdida de datos

Monitoreo real de 5 minutos sobre la misma consulta (`consulta_id=39`, paciente 903), cada ~15-20s, por API directa (`getFilledForm`) y por UI (recargando el perfil del paciente):

- **A los 65s** (donde el spec había cortado los reintentos): seguía en blanco.
- **Para el primer chequeo de la investigación** (~20-40s después, es decir en algún punto entre los 65s y ~100s tras Finalizar): **Exploración segmentaria (17/17 elementos con dato) y Aparatos y sistemas (31/31 elementos con dato) YA mostraban los valores reales completos.**
- Se mantuvo estable con datos completos en **las 20 vueltas siguientes durante los 5 minutos completos** de monitoreo — no es una recuperación momentánea que luego se pierde de nuevo.
- La UI (perfil del paciente, recargado varias veces durante el monitoreo) **nunca mostró "Cargando preguntas" colgado** — 0 apariciones en todas las vueltas.
...
- [ ] Decidir con Pedro si reportar formalmente a devs: signos vitales con valores clínicamente imposibles (FC/Peso/Talla/Presión/Oxigenación/Frecuencia Respiratoria) se guardan sin validación de rango — confirmado con evidencia dura el 2026-08-17 (`registerVitalSigns` 200 con FC=999). Candidato de severidad alta, ver sección "🔍 Corrida completa de la suite dev — 2026-08-17".
- [ ] Decidir con Pedro si reportar formalmente a devs: borrar todas las dosis de la cartilla de Vacunación y guardar ya no persiste (regresión vs. 2026-07-21) — confirmado el 2026-08-17, ver sección homónima. Antes de reportar, valdría repetir la corrida para confirmar que no fue un evento puntual.
- [ ] Investigar con un script dedicado (tipo `_investigar_getfilledform_blanco_dev.js`) el hallazgo abierto de "Detalle de ingreso" atascado en skeleton tras abrir un ingreso pendiente (2026-08-17) — no se pudo reproducir en el momento porque los datos de dev cambiaron; podría ser el mismo patrón de lentitud ya conocido o una reaparición del bug `DetallePagos`.
- [ ] Reportar a devs (baja prioridad, cosmético): la CSP de dev bloquea el propio script/píxel de Microsoft Clarity (dominios `scripts.clarity.ms`/`c.clarity.ms` no están en la política, que solo permite `www.clarity.ms`) — Clarity nunca recolecta analítica en dev. Ver sección "🔍 Corrida completa de la suite dev — 2026-08-17".
- [ ] Si se retoma `consultation.user-errors.spec.js`, reescribirlo para el "Modo Completo" (el helper `iniciarConsultaDeHoy` solo llena 2 de ~7 campos de signos vitales, por eso 5/6 tests fallan igual) — trabajo ya identificado desde 2026-07-21, confirmado seguir pendiente el 2026-08-17.
...
- [x] ~~Portar a staging los specs de dev con confianza Alta/Media (`ajustes.servicios`, `dashboard`, `recetas`)~~ — **hecho el 2026-08-03**, ver sección "🧪 Porteo a STAGING — 2026-08-03". Los 3 pasan tras corregir selectores/paciente desactualizados.
- [ ] Decidir con Pedro si reportar formalmente a devs: `ajustes.servicios.spec.ts` — `saveService` responde 200 con éxito pero el servicio nunca aparece en `getServices` después (confirmado 2 veces, 2026-08-03). Falta decidir si amerita una corrida de confirmación adicional antes.
- [ ] Pedro debe completar manualmente el wizard "Configuración de tu cuenta" de la cuenta `dr@rym-solutions.com` en staging (atascada en Paso 3/6) — hace flaky la verificación de KPIs de `dashboard.spec.js` en staging (ver sección "🧪 Porteo a STAGING — 2026-08-03").
