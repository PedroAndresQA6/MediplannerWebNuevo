// Comparativo dev/staging/PRODUCCION del Hallazgo 2 (getProfile) y Hallazgo 3
// (dropdown "Agregar servicios"). Usa el paciente QA dedicado de produccion
// ("Pedro Andrés Quijada Anaya") -- NO se hace nada destructivo, solo se crea
// una cita/consulta de prueba y se abre un combobox, igual que ya se hizo en
// dev y staging.
//
// Cumple CLAUDE.md §0.7: espera real (getConsultation + campo Motivo visible/
// habilitado + overlay de "Recuperando..."/"Cargando..." ausente, SIN catch
// mudo) en vez de reusar el patron viejo y fragil que tiene el spec de
// produccion (`!body.innerText.includes('Cargando información de consulta')`
// + catch mudo, el mismo problema que ya se encontro y corrigio en dev).
const { chromium } = require('@playwright/test');
// OJO: el createAppointment PROPIO de producción (./e2e/utils.js) todavía
// usa el selector viejo del wizard (div.bg-white.shadow-md.rounded.p-5) que
// ya se sabe roto desde 2026-07-09 en dev -- confirmado hoy que el mismo
// problema sigue sin portar a producción. Para este diagnóstico puntual se
// usa la versión YA ARREGLADA de dev en vez de tocar el archivo compartido de
// producción (eso es decisión de Pedro, no de este script).
const { createAppointment } = require('../e2e/utils.js');

const PACIENTE_BUSQUEDA = 'Codigo';
...
(async () => {
  ...
  console.log('Navegando a Dashboard primero para cerrar onboarding/wizard si aparece...');
  await page.goto('/Dashboard');
  await page.waitForLoadState('load').catch(() => {});
  await page.waitForTimeout(2000);
  await saltarOnboardingYWizardConfig(page);

  console.log(`Creando cita para "${PACIENTE_BUSQUEDA}" en PRODUCCIÓN (paciente QA dedicado)...`);
  ...
  await fill(page.locator('input[name="peso"]'), '70');
  await fill(page.locator('input[name*="talla" i]'), '170');
  ...
  if (guardarHabilitado) {
    ...
    console.log('⚠️ "Guardar" de signos vitales no se habilitó — se sigue igual para ver la pantalla de Servicios.');
  }
  ...
  // Espera real de carga (sin catch mudo): getConsultation + campo Motivo
  // visible/habilitado + ningún overlay de "Cargando"/"Recuperando" visible.
  ...
  if (!(await motivoInput.isEnabled())) throw new Error('Campo Motivo visible pero deshabilitado -- la consulta no terminó de cargar');
  const overlayCarga = page.getByText(/cargando|recuperando/i);
  if (await overlayCarga.isVisible().catch(() => false)) {
    console.log('⏳ Overlay de carga visible, esperando a que desaparezca...');
    await overlayCarga.waitFor({ state: 'hidden', timeout: 30000 });
  }
  await page.waitForTimeout(1000);
  console.log('✅ Consulta cargada de verdad.');
  ...
  console.log('\n--- Abriendo el combobox "Agregar servicios" en PRODUCCIÓN ---');
  ...
  await page.screenshot({ path: 'test-results/recon-servicios-dropdown-produccion.png', fullPage: true }).catch(() => {});

  // NO cerrar/borrar nada -- solo se deja la consulta creada, no destructivo,
  // patrón ya usado antes en producción con este mismo paciente QA.
